...
def inference_one(model, img, output_path):
    # 应用转换
    ...
    # 保存图片
    # cv2.putText(img, f'Predicted: {classes[predicted.item()]}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
    #             (0, 255, 0), 2)
    # cv2.imwrite(output_path, img)
    return predicted.item()


img_folder = '/Users/wengyang/datasets/dogs'
...
for file in tqdm(os.listdir(img_folder)):
    ...
    pred = inference_one(resnet18, img, output_file)
    if pred == true_value:
        correct_num += 1
